...
if not DATABASE_URL:
    raise ValueError("DATABASE_URL not found in environment variables. Check Render settings.")
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS clientes
                 (numero TEXT PRIMARY KEY, ideia TEXT, tamanho_local TEXT, pagamento TEXT, data TIMESTAMP, consentimento TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS agenda
                 (id SERIAL PRIMARY KEY, dia TEXT, horario TEXT, numero TEXT, ideia TEXT, status TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS conversations
                 (numero TEXT PRIMARY KEY, step INTEGER, data TEXT, autenticado BOOLEAN DEFAULT FALSE)''')
    conn.commit()
    conn.close()

# Salvar dados do cliente (PostgreSQL)
def save_to_db(from_number, ideia, tamanho_local, pagamento, consent):
    ...
    c.execute("INSERT INTO clientes (numero, ideia, tamanho_local, pagamento, consent, data) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (numero) DO UPDATE SET ideia = EXCLUDED.ideia, tamanho_local = EXCLUDED.tamanho_local, pagamento = EXCLUDED.pagamento, consent = EXCLUDED.consent, data = EXCLUDED.data",
...
def update_consent(from_number, consent):
    conn = psycopg2.connect(DATABASE_URL)
    c = conn.cursor()
    c.execute("UPDATE clientes SET consentimento = %s WHERE numero = %s", (consent, from_number))
    conn.commit()
    conn.close()

# Salvar agendamento (PostgreSQL)
def save_agendamento(from_number, data, dia, horario, tatuador):
    conn = psycopg2.connect(DATABASE_URL)
    c = conn.cursor()
    c.execute("INSERT INTO agenda (dia, horario, numero, ideia, status) VALUES (%s, %s, %s, %s, %s)",
              (dia, horario, from_number, data['ideia'], 'Pendente'))
    conn.commit()
    conn.close()
...
def visualizar_agenda(com_indices=False):
    ...
    for row in rows:
        ...
        if dia not in agenda:
            agenda[dia] = []
        info = f"{row[2]} - {row[3]} ({row[4]}) - Status: {row[5]}"
...
def remover_agendamento(indice, agendamentos_lista):
    if 0 <= indice < len(agendamentos_lista):
        ...
        conn = psycopg2.connect(DATABASE_URL)
        c = conn.cursor()
        c.execute("DELETE FROM agenda WHERE id = %s", (id_to_remove,))
        conn.commit()
        conn.close()
...
def save_state(from_number, state):
    conn = psycopg2.connect(DATABASE_URL)
    c = conn.cursor()
    c.execute("INSERT INTO conversations (numero, step, data, autenticado) VALUES (%s, %s, %s, %s) ON CONFLICT (numero) DO UPDATE SET step = EXCLUDED.step, data = EXCLUDED.data, autenticado = EXCLUDED.autenticado",
...
def load_state(from_number):
    conn = psycopg2.connect(DATABASE_URL)
    c = conn.cursor()
    c.execute("SELECT step, data, autenticado FROM conversations WHERE numero = %s", (from_number,))
    row = c.fetchone()
    conn.close()
    if row:
